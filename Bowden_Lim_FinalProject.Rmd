...
#Note: these are from the videos, but online says we actually are supposed
#run this test on the residuals when doing a two way as opposed to the 
#raw data...

#Sorptivity
shapiro.test(all_hp$Sorptivity..mm.per.sqrt.hr....DL[all_hp$Site == "BF1"])
shapiro.test(all_hp$Sorptivity..mm.per.sqrt.hr....DL[all_hp$Site == "BF3"])
shapiro.test(all_hp$Sorptivity..mm.per.sqrt.hr....DL[all_hp$Site == "BF5"])

qqnorm(all_hp$Sorptivity..mm.per.sqrt.hr....DL); qqline(all_hp$Sorptivity..mm.per.sqrt.hr....DL)

bartlett.test(all_hp$Sorptivity..mm.per.sqrt.hr....DL ~ all_hp$Site)

#Kfs
shapiro.test(all_hp$Kfs..mm.per.hr...DL[all_hp$Site == "BF1"])
shapiro.test(all_hp$Kfs..mm.per.hr...DL[all_hp$Site == "BF3"])
shapiro.test(all_hp$Kfs..mm.per.hr...DL[all_hp$Site == "BF5"])

#BD 0 to 1
shapiro.test(all_bd$X0.1.0.cm[all_bd$Site == "BF1"])
...
leveneTest(kfs.anova.2way)  #p = 0.0016. should use Welch's ANOVA or log/sqrt transform
leveneTest(sorptivity.anova.2way)

#outliers
plot(kfs.anova.2way, which = 5) #observation 310 is an outlier
...
#homogeneity of variances
leveneTest(ps.g.anova.2way) #p-value = 7.0882 * 10^-11. should log/sqrt transform
leveneTest(ps.s.anova.2way) #p-value = 0.0002052. should loq/sqrt transform
leveneTest(ps.sc.anova.2way) #p-value = 7.027 * 10^-7. should log/sqrt transform
