...
all_ps_new <- all_ps %>%
  mutate(sqrt.gravel = sqrt(Gravel....)) %>%
  mutate(sqrt.sand = sqrt(Sand....)) %>%
  mutate(sqrt.silt.clay = sqrt(Silt...Clay....))
...
#gravel
ps.g.anova.2way <- aov(data = all_ps_new, sqrt.gravel ~ Site * Year)
summary(ps.g.anova.2way)
TukeyHSD(ps.g.anova.2way)
ps.g.interaction <- with(all_ps_new, interaction(Site, Year))
ps.g.anova.2way2 <- aov(data = all_ps_new, sqrt.gravel ~ ps.g.interaction)
ps.g.groups <- HSD.test(ps.g.anova.2way2, "ps.g.interaction", group = TRUE)
ps.g.groups
ps.g.anova.plot <- ggplot(all_ps_new, aes(y = sqrt.gravel, x = Site, color = Year)) +
  geom_boxplot()
print(ps.g.anova.plot)

#sand
ps.s.anova.2way <- aov(data = all_ps_new, sqrt.sand ~ Site * Year)
summary(ps.s.anova.2way)
TukeyHSD(ps.s.anova.2way)
ps.s.interaction <- with(all_ps_new, interaction(Site, Year))
ps.s.anova.2way2 <- aov(data = all_ps_new, sqrt.sand ~ ps.s.interaction)
ps.s.groups <- HSD.test(ps.s.anova.2way2, "ps.s.interaction", group = TRUE)
ps.s.groups
ps.s.anova.plot <- ggplot(all_ps_new, aes(y = sqrt.sand, x = Site, color = Year)) +
  geom_boxplot()
print(ps.s.anova.plot)

#silt/clay
ps.sc.anova.2way <- aov(data = all_ps_new, sqrt.silt.clay ~ Site * Year)
summary(ps.sc.anova.2way)
TukeyHSD(ps.sc.anova.2way)
ps.sc.interaction <- with(all_ps_new, interaction(Site, Year))
ps.sc.anova.2way2 <- aov(data = all_ps_new, sqrt.silt.clay ~ ps.sc.interaction)
ps.sc.groups <- HSD.test(ps.sc.anova.2way2, "ps.sc.interaction", group = TRUE)
ps.sc.groups
ps.sc.anova.plot <- ggplot(all_ps_new, aes(y = sqrt.silt.clay, x = Site, color = Year)) +
  geom_boxplot()
